...
import com.google.protobuf.TextFormat; // Importa a classe TextFormat do pacote com.google.protobuf
...
public class Util {
    ...
    public static Date dataAtual() {
        ...
        return Date.valueOf(hoje); // Retorna a data atual no formato SQL
    }
    ...
    public static Date stringParaDateSQL(String dataString) throws TextFormat.ParseException, java.text.ParseException {
        ...
        java.util.Date dataUtil = formato.parse(dataString); // Converte a string para um objeto Date do Java
        return new Date(dataUtil.getTime()); // Retorna a data no formato SQL
    }
    
    // Método para formatar um número de telefone
    public static String formatarNumero(String numero) {
        Pattern pattern = Pattern.compile("(\\d{2})(\\d{5})(\\d{4})"); // Define o padrão do número de telefone
        Matcher matcher = pattern.matcher(numero); // Cria um objeto Matcher para encontrar padrões na string

        if (matcher.matches()) { // Se o padrão for encontrado na string
            String numeroFormatado = "(" + matcher.group(1) + ")" + matcher.group(2) + "-" + matcher.group(3); // Formata o número de telefone
            return numeroFormatado; // Retorna o número de telefone formatado
        } else { // Se o padrão não for encontrado na string
            return numero; // Retorna a string original
        }
    }
}
